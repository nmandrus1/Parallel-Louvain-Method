...
DistCommunities::DistCommunities(Graph &g) : g(g) {
  ...
  for (int v = g.rows.first; v < g.rows.second; v++) {
    ...
    comm_size[v] = 1;
    total[v] =
        g.degree(v); // Total degree of the community is the degree of the node.
...
void DistCommunities::insert(int node, int community, int degree,
  ...
  if (comm_size[community] == 1 | comm_size[community] == 0)
    in[community] = 0;
...
bool DistCommunities::iterate() {
  ...
  // negative for exponential decay of temperature as a function of the number
  // of iterations over every vertex. Higher the number of iterations, the less
  // likely it should be for vertices to move around
  int iteration = -1;
  double temperature;
...
// std::cout << "RANK " << g.info->rank << ": Computing neighbors" << std::endl;
      ...
      // Temporarily remove the node from its current community.
      // std::cout << "RANK " << g.info->rank << ": Removing vtx " << vtx << "
      // from comm " << vtx_comm << std::endl;
      ...
      // std::cout << "RANK " << g.info->rank << ": computing best comm" <<
      // std::endl;
      ...
      if (best_comm != vtx_comm) {
        // std::cout << "RANK " << g.info->rank << ": best comm = " << best_comm
        // << std::endl;
        ...
        // std::cout << "RANK " << g.info->rank << ": Communicating
        // removal/addition \t old: " << old_comm_owner << " new: " <<
        // new_comm_owner << std::endl;
...
// NOTE: subject to change if vertex rejections are implemented
      ...
      GPTLstart("insert");
#endif
      insert(vtx, best_comm, g.degree(vtx), edges_to_other_comms[best_comm],
             vtx_rank_degree[vtx]);
#ifdef PROFILE_FNS
      GPTLstop("insert");
#endif
...
void DistCommunities::process_incoming_updates() {
  ...
  while (true) {
    ...
    if (flag) {
      receive_community_update(status.MPI_SOURCE, status, update, buf);
...
void DistCommunities::update_neighbors(int vtx) {
  ...
  // loop over all the ranks that border this vtx and send them neighbor
  // information
  for (auto &[rank, count] : vtx_rank_degree[vtx]) {
    buf.push_back(vtx);
    buf.push_back(gbl_vtx_to_comm_map[vtx]);

    MPI_Send(buf.data(), buf.size(), MPI_INT, rank, MPI_NEIGHBOR_SYNC,
             MPI_COMM_WORLD);
    buf.clear();
  }
  ...
  // Check for incoming community updates
  while (true) {
    MPI_Iprobe(MPI_ANY_SOURCE, MPI_NEIGHBOR_SYNC, MPI_COMM_WORLD, &flag,
               &status);
    ...
    if (flag) {
      ...
      MPI_Recv(buf.data(), size, MPI_INT, status.MPI_SOURCE, MPI_NEIGHBOR_SYNC,
...
int DistCommunities::compute_best_community(int node, int node_comm,
  ...
  for (auto neighbor_comm : neighbor_comms) {
    ...
    if (increase > best_increase &&
        std::abs(best_increase - increase) > temperature) {
      best_comm = neighbor_comm;
      best_increase = increase;
    }
  }
...
void DistCommunities::send_community_update(int dest,
                                            const CommunityUpdate &update) {
  int position = 0;
  int num_ranks_size = 2 * update.num_ranks_bordering_node * sizeof(int);
  int total_size = 0;
  MPI_Pack_size(6, MPI_INT, MPI_COMM_WORLD,
                &total_size);   // For node, global_degree, old_comm, new_comm,
                                // num_ranks_bordering_node, and type
  ...
  MPI_Pack(&update_type, 1, MPI_INT, buffer, total_size, &position,
           MPI_COMM_WORLD);

  MPI_Pack(&update.node, 1, MPI_INT, buffer, total_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(&update.global_degree, 1, MPI_INT, buffer, total_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(&update.old_comm, 1, MPI_INT, buffer, total_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(&update.new_comm, 1, MPI_INT, buffer, total_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(&update.edges_within_old_comm, 1, MPI_DOUBLE, buffer, total_size,
           &position, MPI_COMM_WORLD);
  MPI_Pack(&update.edges_within_new_comm, 1, MPI_DOUBLE, buffer, total_size,
           &position, MPI_COMM_WORLD);
  MPI_Pack(&update.num_ranks_bordering_node, 1, MPI_INT, buffer, total_size,
           &position, MPI_COMM_WORLD);
  ...
  // std::cout << "RANK " << g.info->rank << ": Sending packed update to rank "
  // << dest << std::endl;
}
...
void DistCommunities::receive_community_update(
  ...
  MPI_Recv(buffer, count, MPI_PACKED, source, MPI_DATA_TAG, MPI_COMM_WORLD,
           &status);

  int position = 0;
  int update_type;
  MPI_Unpack(buffer, count, &position, &update_type, 1, MPI_INT,
             MPI_COMM_WORLD);
  update.type = static_cast<CommunityUpdate::UpdateType>(update_type);

  MPI_Unpack(buffer, count, &position, &update.node, 1, MPI_INT,
             MPI_COMM_WORLD);
  MPI_Unpack(buffer, count, &position, &update.global_degree, 1, MPI_INT,
             MPI_COMM_WORLD);
  MPI_Unpack(buffer, count, &position, &update.old_comm, 1, MPI_INT,
             MPI_COMM_WORLD);
  MPI_Unpack(buffer, count, &position, &update.new_comm, 1, MPI_INT,
             MPI_COMM_WORLD);
  MPI_Unpack(buffer, count, &position, &update.edges_within_old_comm, 1,
             MPI_DOUBLE, MPI_COMM_WORLD);
  MPI_Unpack(buffer, count, &position, &update.edges_within_new_comm, 1,
             MPI_DOUBLE, MPI_COMM_WORLD);
  MPI_Unpack(buffer, count, &position, &update.num_ranks_bordering_node, 1,
             MPI_INT, MPI_COMM_WORLD);

  rank_borders_buf.resize(2 * update.num_ranks_bordering_node);
  MPI_Unpack(buffer, count, &position, rank_borders_buf.data(),
             2 * update.num_ranks_bordering_node, MPI_INT, MPI_COMM_WORLD);
}
...
void DistCommunities::print_comm_membership() {
  ...
  for (int i = 0; i < g.info->comm_size; i++) {
    if (g.info->rank == i) {
      for (int v = g.rows.first; v < g.rows.second; v++)
        std::cout << "RANK " << g.info->rank << ": Vtx " << v
                  << " Community: " << gbl_vtx_to_comm_map[v] << "\n";
      std::cout << std::endl;
    }
...
void DistCommunities::write_communities_to_file(const std::string &directory) {
  ...
  if (g.info->rank == 0) {
    std::cout << "All ranks have finished writing community data to "
              << directory << std::endl;
  }
}
